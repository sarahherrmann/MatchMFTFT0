...
struct nchMft {
  ...
  {
    //look at the MFT tracks matched with forward muon detectors
    //and study the quality of MFT-muon matching via the MFT BCs (BCs obtained with MFT_FT0-C matching)
    for (auto& mfttrack : mfttracks)
    {
      ...
      auto matchedFwdtrack = mfttrack.fwdtrack_as<FwdTracksWColls>();//the forward track matched with the current mfttrack
      if (!isMuonSelected(matchedFwdtrack))
      {
        continue;//only looking at "believable" muons and no ambiguous forward tracks
      }
      ...
      if (!matchedFwdtrack.collision_as<CollwEv>().sel8()){
        continue;//only looking at muon's collision selected by event selection
      }
      ...
      {//
        int64_t bcDiff = bc.globalBC() - collFwdBC;//difference between the muon's BC and the MFT track's BC
